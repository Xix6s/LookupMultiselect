...
const MultiselectRecords = (props: IMultiselectProps) => {
    ...
    useEffect(() => {
        const errorMessageText = isError === 0 ? "" : isError === -1 ? `No Records Found.` : `No Records Found.`;
    ...
    const setColumnGroups = async (recordsPassedParam: any = null) => {
        ...
        let distinctGroups = recordsPassed.reduce((currentGroups: any, currentItem: { [x: string]: any; }, index: any) => {
            let lastGroup = currentGroups[currentGroups.length - 1];
            let fieldValue = currentItem[columnKey] !== undefined ? currentItem[columnKey] : currentItem['srm_name'] + ' - PARENT';
                    ...
                    <Stack gap="5" horizontal wrap maxWidth={props.widthProp}>
                        {myItems != null && myItems.length > 0 && myItems.map((item: any) => {
                            ...
                            if (!props.isControlDisabled) {
                                return (
                                    <Stack horizontal style={{ border: "1px solid #106EBE", marginTop: "20px", background: "rgb(0 120 212)"}}>
                                        <PrimaryButton
                                            primary className="buttonContainer"
                                            text={theItem.name}
                                        ...
                                        />
                                        <IconButton primary iconProps={clearIcon} title="Clear" ariaLabel="Clear" onClick={removeFieldValue} data-custom-id="button-custom-clear" style={{ background: "rgb(0 120 212)", color: "white", fontSize: "8px", lineHeight: "8px" }} />
                                    </Stack>
                                )
                            } else {
                                return (
    ...
    const _showSearchTextField = (): JSX.Element => {
        if (props.isControlVisible && !props.isControlDisabled) {
            return (
                <>
                    <div style={{ color: "rgb(0, 0, 255)", marginTop: "10px", marginRight: "10px" }}>+</div>
                    <TextField className={"text"}
                ...
                <Stack style={{ marginTop: "-50px;", margin: "auto" }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', flex: 1, alignContent: 'space-between' }}>
                        <CommandBarButton
                            ...
                            iconProps={acceptIcon} text="APPLY" onClick={setFieldValue} styles={{
                                root: {
                                    flex: 1,
                                    padding: 10,
                                    zIndex: 1995,
                        ...
                        <CommandBarButton
                            ...
                            iconProps={clearIcon} text="CANCEL" onClick={clearItems} styles={{
                                root: {
                                    flex: 1,
                                    padding: 10,
                                    zIndex: 1995,
                    ...
                    <Sticky stickyPosition={StickyPositionType.Header} isScrollSynced={true} >
                        <Stack grow verticalFill className="container" style={{ width: props.widthProp, position: 'relative' }}>
                            <Stack.Item grow className="gridContainer" style={{ overflowY: 'auto', overflowX: 'auto', position: 'relative' }}>
                                <ScrollablePane className="scrollableContainer" scrollbarVisibility={ScrollbarVisibility.auto} style={{ position: 'relative' }}>
                                    <DetailsList
    ...
    const filterRecordsClick = async () => {
        ...
        if (recordsRetrieved != -1 && recordsRetrieved != -2) {
            ...
            let recordsRetrievedSorted: any = _sortColumns(recordsSortedByName, props.groupBy);
            ...
            for (let item of selectedItems) {
                const indexItem = recordsRetrievedSorted.findIndex((x: any) => item[props.attributeid] == x[props.attributeid])
                if (indexItem != -1) {
    ...
    /**
        ...
        */
    return (
        <div className={"divContainer"} style={{ marginTop: textFieldValue !== "" && textFieldValue !== "[]" ? "40px" : "0px" }}>
