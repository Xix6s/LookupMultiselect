...
export class LookupMultiselect
  implements ComponentFramework.StandardControl<IInputs, IOutputs>
{
  ...
  public init(
    ...
    const columns: string =
      this._context.parameters.columns.raw?.trim() || "Name,name";
    ...
    this.props.attributeid =
      this._context.parameters.attributeid.raw || "accountid";
    ...
    this._populatedFieldVisible =
      this._context.parameters.populatedFieldVisible.raw || "False";
    this.props.populatedFieldVisible =
      this._populatedFieldVisible == "True" ? true : false;

    this.props.logicalName = this._entityName;
    this.props.openWindow =
      this._context.parameters.openFormOptions.raw || "Pop up";
  ...
  private async retrieveRecordsFromFetch(input: string): Promise<any> {
    if (this._isFetchXml == true) {
      this._records = await MultiselectModel.GetDataFromApiWithFetchXml(
        this._context,
        this._entityName,
        this._filter,
        this._numberOfRecordsToBeShown
      );
    } else {
      this._records = await MultiselectModel.GetDataFromApi(
        this._context,
        this._entityName,
        this._filter,
        this._numberOfRecordsToBeShown
  ...
  private async filteredUrlFromDynamicValues(_filter: string): Promise<string> {
    ...
    const result = await MultiselectModel.GetDataFromEntity(
      this._context,
      this._entityRecordName,
      this._entityRecordId,
      this._filterDynamicValues
    );
  ...
  private renderElement(): void {
    ReactDOM.render(
      React.createElement(MultiselectRecords, this.props),
      this._container
    );
  }
